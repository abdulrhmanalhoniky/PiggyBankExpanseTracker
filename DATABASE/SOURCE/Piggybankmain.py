...
from utlity import is_valid_email, upload_to_telegram
...
class PiggyBankLauncher(MDApp):
    ...
    def start_backup_process(self, instance):
        self.status_label.text = "will be uploading the database to telegram, please wait..."
        self.backup_btn.disabled = True

        threading.Thread(target=self.run_utility_task).start()

    def on_stop(self):
        print("App is stopping, starting backup process...")
        upload_to_telegram()

    def finish_up(self, success, message):
        self.status_label.text = message
        self.backup_btn.disabled = False
        self.backup_btn.text = "Try Again" if not success else "Uploaded to Telegram"
